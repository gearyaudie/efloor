...
export default function ClosingCta({
  ...
  lede = "Dapatkan harga terbaik untuk retail, kontraktor, maupun procurement. Tim kami membalas di jam kerja.",
  source = "home-closing",
  product,
}: {
  title?: string;
  lede?: string;
  /** Analytics source for every link in the block. */
  source?: string;
  product?: string;
} = {}) {
  return (
    <section className="max-w-[1200px] mx-auto px-4 md:px-8 pb-[72px] lg:pb-[104px]">
      ...
      >
        ...
        <address className="not-italic rounded-3xl p-6 bg-white/15 backdrop-blur-md shadow-[inset_0_0_0_1px_rgba(255,255,255,0.25)] text-[14.5px] leading-relaxed">
          ...
          <PhoneLink source={source} className="font-mono underline-offset-4 hover:underline">
            {PHONE_DISPLAY}
          </PhoneLink>
        </address>
      </div>
