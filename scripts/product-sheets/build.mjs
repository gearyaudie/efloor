...
function sheet(config, variants, img) {
  ...
  const specs = config.specs.filter(([k]) => k !== "Produk");
...
<section class="page">
  ...
  <p class="note">Harga per batang (${config.lengthCm} cm) dalam Rupiah, dapat berubah sewaktu-waktu. Pembelian proyek &amp; grosir: minta harga khusus via WhatsApp.</p>

  <h2><span class="bar"></span>Warna</h2>
  <div class="colorrow">
    <img class="colors" src="${img.colors}" alt="">
    <p class="small">${esc(config.colorsNote)}</p>
  </div>
...
.colorrow { display: grid; grid-template-columns: 1.6fr 1fr; gap: 5mm; align-items: center; }
.colorrow .small { margin-top: 0; }
.colors { width: 100%; height: 24mm; object-fit: cover; border-radius: 4mm; }
...
.contact { margin-top: auto; flex-shrink: 0; display: grid; grid-template-columns: 1.1fr 1fr; gap: 5mm; background: linear-gradient(118deg, #ff8e06, #f2561d 55%, #c62020); color: #fff; border-radius: 5mm; padding: 5mm 6mm; }
.cta b { display: block; font-size: 13pt; letter-spacing: -0.01em; }
.cta a { display: inline-block; margin-top: 2.5mm; margin-right: 2mm; color: #18171b; background: #fff; text-decoration: none; font-weight: 600; font-size: 9pt; padding: 1.8mm 4mm; border-radius: 99px; }
.addr { font-size: 8.3pt; line-height: 1.4; background: rgba(255,255,255,.15); border-radius: 3.5mm; padding: 3.5mm 4mm; }
.addr small { display: block; font-size: 6.8pt; text-transform: uppercase; letter-spacing: .1em; opacity: .85; font-weight: 600; margin-top: 1.8mm; }
...
for (const key of Object.keys(FILES)) {
  ...
  const img = { logo, hero: await dataUri(config.gallery[0].src), colors: await dataUri(config.colorPhoto.src) };
...
${sheet(config, variants, img)}</body></html>`;
  ...
  if (process.env.DEBUG) console.log(await page.evaluate(() => [...document.querySelector(".page").children].map((c) => `${c.className || c.tagName}:${Math.round(c.getBoundingClientRect().height)}`).join(" ")));
